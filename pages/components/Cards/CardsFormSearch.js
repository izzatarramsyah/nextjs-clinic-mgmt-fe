...
export default function CardFormSearch({ columnTransaction, handleSearch, isLoading, transaction, transactionType }) {
  ...
  const doExport = async(e) => {
    e.preventDefault();
    const worksheet = XLSX.utils.json_to_sheet(columnTransaction);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");
    XLSX.writeFile(workbook, 'Data Pasien.xlsx');
  }
              ...
              <form>
                  ...
                  <div className="w-full lg:w-12/12 px-4">
                    <div className="relative w-full mb-6">
                      <label
                        ...
                        htmlFor="grid-password" > Jenis Transaksi
                      </label>
                      <select onChange={(e)=> handleChange(e)}
                        className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                        {transaction?.map(option => (
                          <option key={option.value} value={option.value} disabled={option.isDisabled} selected={option.isSelected}>
                            {option.text}
                          </option>
                        ))}
                      </select>
                ...
                <div className="flex justify-end">                  
                  ...
                  { (  isLoading ? 
                    ...
                    <button
                      className="bg-[#68788A] active:bg-[#68788A] text-white font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150"
